...
void startTimer() { startTime = Sys::millis(); }
...
//____________________________________________________________________
//
EventFilter* EventBus::firstFilter() { return _firstFilter; }
//____________________________________________________________________
//
...
void startHeap() {
  if (Sys::getFreeHeap() < lastHeapSize) lastHeapSize = Sys::getFreeHeap();
}
...
void stopHeap(Actor* actor, const char* s) {
  ...
  if (heapSize < lastHeapSize) {
    if (actor) {
      INFO(" HEAP : %d:%d in actor : %s : %s ", heapSize,
           heapSize - lastHeapSize, actor->name(), s);
    }

    else {
      INFO(" HEAP :  %d:%d in static function :%s ", heapSize,
           heapSize - lastHeapSize, s);
    }
  }
}
  ...
  if ((_queue.get(_rxd) == 0)) {  // handle all events
    ...
    for (EventFilter* filter = firstFilter(); filter;
      ...
      if (filter->match(_rxdHeader)) {
        for (Subscriber* sub = filter->firstSubscriber(); sub != 0;
             sub = sub->next()) {
          startTimer();
          if (sub->_actor == 0) {
            startHeap();
...
//____________________________________________________________________
//
void EventFilter::call(Actor* instance) { call(instance, 0); }

EventFilter* EventFilter::next() { return _nextFilter; }
...
//____________________________________________________________________
//
Subscriber* EventFilter::firstSubscriber() { return _firstSubscriber; }
Subscriber* Subscriber::next() { return _nextSubscriber; }
//____________________________________________________________________
//
